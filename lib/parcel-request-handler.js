...
exports.registerAddress = function (req, res){
  ...
  util.getUserFromToken(req, function (authUser) {
    ...
    planner.registerAddress(req.body, user_id, function (err, plannerData) {
      console.log(err);
      console.log(plannerData)
      if (err || plannerData.name ==='error') {
        return res.status(404).end('invalid times');
      } 
  ...
  new Parcel({'gid': gid}).fetch().then(function(parcel){
      if (!parcel) {
        res.status(404).end('Failure to find parcel');
          ...
          planner.removeAddress(gid, function (err, plannerResponse) {
...
exports.setExemption = function (req, res) {
  ...
  planner.setExemption(req.body, function (err, plannerResponse) {
    if (err || plannerResponse.name ==='error') {
  ...
  new Exemption({'exemption_id': exemption_id}).fetch().then(function(exemption){
      if (!exemption) {
        res.status(404).end('Failure to find exemption');
          ...
          planner.removeExemption(exemption_id, function (err, plannerResponse) {
            if (err || plannerResponse.name ==='error') {
              return res.status(404).end('invalid exemption request');
            } 
